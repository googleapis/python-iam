...
def test_list_applicable_policies(request_type, transport: str = "grpc"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
        type(client.transport.list_applicable_policies), "__call__"
    ) as call:
        # Designate an appropriate return value for the call.
...
@pytest.mark.asyncio
async def test_list_applicable_policies_async(
    transport: str = "grpc_asyncio", request_type=policy.ListApplicablePoliciesRequest
):
    client = PoliciesAsyncClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
...
def test_list_applicable_policies_pager(transport_name: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        metadata = ()
        metadata = tuple(metadata) + (
            gapic_v1.routing_header.to_grpc_metadata((("attachment_point", ""),)),
        )
...
async def test_list_applicable_policies_async_pager():
    ...
    with mock.patch.object(
        ...
        async_pager = await client.list_applicable_policies(
            request={},
        )
        assert async_pager.next_page_token == "abc"
        responses = []
